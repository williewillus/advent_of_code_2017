...
pub fn knot_hash(input: &str) -> String {
    let mut lens = input.bytes().map(|b| b as usize).collect::<Vec<_>>();
    ...
    let mut skip_size = 0;
    for _ in 0..64 {
        hash_iter(&mut data, &lens, &mut pos, &mut skip_size);
    }
...
pub fn run() {
    ...
    println!("part 2: {}", knot_hash(&util::read_all("d10_input.txt").unwrap()));
}
